	...
	$.ajax({
		...
		success: function(jobCodeList) {
			...
			jobDataGroup.forEach(function(group) {
				$('#jobCodeGroupUl').append("<li><input type='checkbox' class='jobCk' value='"+group.code+"/"+group.name+"' id='j"+group.code+"' name='jobCode'><label for='j"+group.code+"'>" + group.name + "</label></li>");
			...
			jobDataSub.forEach(function(sub) {
				$('#jobCodeSubUl'+sub.code.substr(0,2)).append("<li><input type='checkbox' class='jobCk' value='"+sub.code+"/"+sub.name+"' id='j"+sub.code+"' name='jobCode'><label for='j"+sub.code+"'>" + sub.name + "</label></li>");
				$('#jobCodeStepDiv').append("<ul class='jobCodeStepUl' id='jobCodeStepUl"+sub.code+"'></ul>");
				$('#jobCodeStepUl'+sub.code).append("<li><input type='checkbox' class='jobCk jobCk_all' value='"+sub.code+"' id='j_all_"+sub.code+"' name='jobCode'><label for='j_all_"+sub.code+"'>전체</label></li>");
				$('#jobCodeStepUl'+sub.code).hide();
			});
			jobDataStep.forEach(function(step) {
				$('#jobCodeStepUl'+step.code.substr(0,5)).append("<li><input type='checkbox' class='jobCk jobCk_step' value='"+step.code+"/"+step.name+"' id='j"+step.code+"' name='jobCode'><label for='j"+step.code+"'>" + step.name + "</label></li>");
			});
		},
