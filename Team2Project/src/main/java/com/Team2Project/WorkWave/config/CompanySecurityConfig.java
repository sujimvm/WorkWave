	...
	public SecurityFilterChain companySecurityFilterChain(HttpSecurity http) throws Exception {
		...
		http
            ...
            .requestMatchers("/main.go","/", "/index","/company_insert.go","/idcheck.go",
            				 ...
            				 "/checkUserId","/sendCode","/verifyCode","/comNoCheck.go").permitAll()
            .requestMatchers("/resources/**", "/css/**", "/js/**", "/image/**").permitAll()
            // 여기는 기업이나 개인 롤을 가진 계정만 접근 가능
            .requestMatchers("/comBoard/").hasAnyRole("COMPANY","USER") 
            // 여기는 기업 롤을 가진 계정만 접근가능
            .requestMatchers("/comBoard/add").hasRole("COMPANY") 
            // 여기는 개인 롤을 가진 계정만 접근가능
            .requestMatchers("적어야함").hasRole("USER")
            // 다른 요청은 인증 필요
